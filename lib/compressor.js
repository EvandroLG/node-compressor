...
Compressor.prototype = {
  ...
  createDirectories: function(callback) {
    ...
    fs.mkdirSync(path.join(basedir, 'js'));
    fs.mkdirSync(path.join(basedir, 'css'));
  ...
  createMinifyJs: function(scripts) {
    var rootedScripts = scripts.map(function(script) {
      return path.join(this.root, script);
    }.bind(this));

    var code = uglify.minify(rootedScripts).code;
    var filename = path.join('js', guid() + '.js');
  ...
  createMinifyCss: function(styles) {
    ...
    styles.forEach(function(src, index) {
      var currentStyle = path.join(root, src);
      ...
      fs.readFile(currentStyle, 'utf-8', function(err, data) {
        ...
        var srcStyle = path.join(root, '.compressed', filename);
